...
<div class="col-sm-2 p-0 mainbody">
    ...
    <?php
		  extract($_POST);
            $stuid = $_SESSION["id"];
  //          echo  $stuid ;
        	$sql = "SELECT COUNT(record) AS Maxrecord  FROM student WHERE studentid='$stuid'";
...
<table class="table mt-1">
	...
	<?php
			...
			do{
				if ($count == 0){
				$sql = "SELECT * FROM student WHERE studentid='$stuid' ORDER BY record ASC  LIMIT 1 ";
				$ra = mysqli_query($conn, $sql) or die(mysqli_error($conn));
				$arow = mysqli_fetch_assoc($ra);
				if (isset($arow["record"]) == 1 ){
				$id = $arow["studentid"] ;}}else{
				$sql = "SELECT * FROM student WHERE studentid='$stuid' ORDER BY record ASC  LIMIT $count,1 ";
				$ra = mysqli_query($conn, $sql) or die(mysqli_error($conn));
				$arow = mysqli_fetch_assoc($ra);}	
				if (isset($arow["record"]) == 1 ){
				$id = $arow["studentid"] ;}
