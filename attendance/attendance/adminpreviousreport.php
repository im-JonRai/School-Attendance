...
<html lang="en">
   ...
   <body>
         ...
         <div class="col-5 p-0 search">
            <?php
               extract($_POST);
                       ...
                       $Maxid = $rsmax["Maxid"] ;
                       $sql = "SELECT COUNT(class_id) AS maxclass FROM class WHERE `date` not between '$YEARstart' AND '$YEAREND'  ";
                       $smax = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                       $rsmax = mysqli_fetch_assoc($smax);
     ...
     //                  echo  $maxclass;
               ...
               do{
               if ($countclass == 0){
               $sql = "SELECT * FROM class WHERE `date` not between '$YEARstart' AND '$YEAREND' ORDER BY class_id ASC  LIMIT 1 ";
               ...
               $arow = mysqli_fetch_assoc($ra);
                           ...
                           elseif ($countclass >= 0){
               $sql = "SELECT * FROM class WHERE `date` not between '$YEARstart' AND '$YEAREND' ORDER BY class_id ASC  LIMIT $countclass,1 ";
               $ra = mysqli_query($conn, $sql) or die(mysqli_error($conn));
               $arow = mysqli_fetch_assoc($ra);
                           $class = $arow["class_name"] ;
            ...
            <table class="table mt-1">
               ...
               <?php   if (empty($stack)) {}
                 ...
                 //     echo $stackno;
                  ...
                  if ($count == 0){
                  $sql = "SELECT * FROM student WHERE class='$stack[$counter]' ORDER BY record ASC  LIMIT 1 ";
                  $ra = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                  $arow = mysqli_fetch_assoc($ra);
                  if (isset($arow["record"]) == 1 ){
                  $id = $arow["studentid"] ;}}
                      else{
                  $sql = "SELECT * FROM student WHERE class='$stack[$counter]' ORDER BY record ASC  LIMIT $count,1 ";
                  $ra = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                  $arow = mysqli_fetch_assoc($ra);}	
